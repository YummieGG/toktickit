...
export function StaffTicketDetail() {
  ...
  const saveStatus = (status: TicketStatus) => {
    if (!ticket || CONFIRM_STATUSES.has(status) && !window.confirm(`Confirm changing this ticket to ${displayStatus(status)}?`)) return;
